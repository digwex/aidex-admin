export const userSortTitles = [
  { label: 'ID', sort: 'nId' },
  { label: 'Telegram ID', sort: 'telegramId' },
  { label: 'ID Реферера', sort: 'referrerId' },
  { label: 'Кол.Реф', sort: 'referralsCount' },
  { label: 'Суб Реф', sort: 'subReferralsCount' },

  { label: 'Ник в телеграм', sort: 'tgUsername' },
  { label: 'Открыл сделок', sort: 'openedTrades' },
  { label: 'Общий бал. SOL', sort: 'balance' },
  { label: 'Получено с комиссий', sort: 'commissions' },
  {
    label: 'PNL',
    sort: 'totalPnl'
  },
  { label: 'Партнёрский тариф', sort: 'referralLevel' },
  { label: 'Кошельки', sort: null },

  { label: 'Опции', sort: null }
]
